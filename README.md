...
- **j / k** (または ↑↓): 選択移動
- **Enter**: ディレクトリに入る / ファイルを `$EDITOR` で開く
...
- シンボリックリンクを `Enter` した場合は参照先に対して処理される
  ...
  - ファイルリンク: 通常ファイルと同じく `$EDITOR` で開く
  - 壊れたリンク: エラーメッセージを表示して処理を中断
...
- `n` で Browse モードから作成フローを開始
- 選択中エントリの直下に名前入力欄が表示される
- 名前の先頭に `/` をつけるとディレクトリ、それ以外はファイルとして作成
- **Enter**: 作成して Browse に戻る
- **Esc**: キャンセルして Browse に戻る
- コマンドモードの `mkdir` はディレクトリ専用。`n` は Browse からワンキーでファイル/ディレクトリ両方を作成可能
